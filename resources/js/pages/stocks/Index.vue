...
function apply(overrides: Record<string, string | null>) {
    ...
    if (q) query.q = q;
    router.get('/stocks', query, { preserveState: true, preserveScroll: true, replace: true });
}
        ...
        <div class="overflow-hidden rounded-xl border border-sidebar-border/70 bg-card dark:border-sidebar-border">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-sidebar-border/70 dark:divide-sidebar-border">
                    <tr v-for="stock in stocks" :key="stock.id" class="transition-colors hover:bg-accent/40">
                        <td class="px-4 py-2.5">
                            <Link :href="`/stocks/${stock.symbol}`" class="flex items-center gap-2 font-semibold text-foreground hover:underline">
                                {{ stock.symbol }}
                                <MarketBadge :market="stock.market" />
                            </Link>
                        </td>
                        <td class="px-4 py-2.5 text-muted-foreground">{{ stock.name }}</td>
                        <td class="px-4 py-2.5 text-right tabular-nums text-foreground">{{ formatPrice(stock.price, stock.currency) }}</td>
                        <td class="px-4 py-2.5 text-right">
                            <PriceChange :change="null" :change-percent="stock.change_percent" :show-absolute="false" />
                        </td>
                        <td class="px-4 py-2.5 text-right">
                            <Link
