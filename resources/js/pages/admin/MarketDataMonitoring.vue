...
const typeLabel = (t: string): string =>
    ({
        nasdaq_list: 'Index constituents',
        ...
        >
            ...
            <div class="grid gap-3 sm:grid-cols-3">
                ...
                >
                    ...
                    >
                        {{ f.status ?? 'never run'
                        }}<span v-if="f.finished_at">
                            · {{ f.finished_at }}</span
                        >
                    </p>
                    <p
                        v-if="f.processed !== null"
                ...
                >
                    <tr v-for="p in providers" :key="p.key">
                        <td class="px-4 py-2.5">
                            ...
                            <span
                                v-if="p.rate_limited"
                                class="ml-2 rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-medium text-amber-700 dark:bg-amber-500/15 dark:text-amber-300"
                                >rate-limited</span
                            >
                        </td>
                        <td class="px-4 py-2.5">
                            <span
                                ...
                                :class="statusClass(p.status_color)"
                                >{{ p.status }}</span
                            >
                            <span
                                v-if="p.consecutive_failures > 0"
                                class="ml-2 text-xs text-rose-600 dark:text-rose-400"
                                >{{ p.consecutive_failures }} fails</span
                            >
                        </td>
                        <td
                            class="px-4 py-2.5 text-right text-muted-foreground tabular-nums"
                        >
                            {{ p.daily_request_count }} /
                            {{ p.daily_failure_count }}
                        </td>
                        <td
                            class="px-4 py-2.5 text-right text-muted-foreground tabular-nums"
                        >
